...
def structure_document(
    ...
    Args:
        text: 원본 텍스트 (줄바꿈 포함)
        doc_id: 문서 고유 ID (예: "src_001", "proc_001")
        title: 문서 제목
        doc_type: 문서 타입 ("source" 또는 "processed")
...
def create_mapping_dict(
    ...
    Args:
        ...
        processed_doc: 처리된 문서 (요약문 등)
        mappings: 기존 매핑 정보 (선택사항)
    
    Returns:
        전체 데이터 구조 (source_documents, processed_document, mappings)
    """
    result = {
...
정기 보안 감사가 필수입니다."""
    ...
    processed_doc = structure_document(
        text=summary_text,
        doc_id="proc_001",
    ...
    print(f"요약문: {len(processed_doc['content']['sentences'])}개 문장")
    print("\n구조화된 데이터 미리보기:")
